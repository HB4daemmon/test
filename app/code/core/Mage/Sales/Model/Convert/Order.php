...
class Mage_Sales_Model_Convert_Order extends Varien_Object
{
    ...
    /**
     * 
     * Convert order object to by store groups orders
     * @param Mage_Sales_Model_Order $order
     */
    
    public function toChildrenOrders(Mage_Sales_Model_Order $order,Mage_Sales_Model_Quote $quote)
    {
    	
    
    	$quote_storegroups=Mage::getModel('sales/quote_storegroup')->getCollection();
    	$quote_storegroups->addFieldtoFilter('quote_id',$quote->getId());
    	
    	//分订单flat_order
    	//分order_storegroup
    	//order_item对应order_storegroup
    	foreach ($quote_storegroups as $key => $quote_storegroup){
    		$child_order=Mage::getModel('sales/order');
    	    $child_order->setIncrementId($quote->getReservedOrderId())
            ->setStoreId($quote->getStoreId())
            ->setQuoteId($quote->getId())
            ->setQuote($quote)
            ->setCustomer($quote->getCustomer())
            ->setData('parent_order_id',$order->getId());
              Mage::helper('core')->copyFieldset('sales_convert_quote', 'to_order', $quote, $child_order);
