 ...
 */
class Luciky_Checkout_Model_Cart extends Varien_Object implements Mage_Checkout_Model_Cart_Interface
{
    /**
    ...
    public function updateItems($data)
    {
        ...
        foreach ($data as $itemId => $itemInfo) {
            ...
            $substitute=$itemInfo['substitute'];
            $customer_message=$itemInfo['customer_message'];
            if ($qty > 0) {
                $item->setQty($qty);
                $item->setSubstitute($substitute);
                $item->setCustomer_message($customer_message);
