 ...
 */
class Luciky_Sales_Model_Quote extends Mage_Sales_Model_Quote
{
	...
	//	Mage::throwException($item->getStoreId());
    ...
    protected function _addCatalogProduct(Mage_Catalog_Model_Product $product, $qty = 1)
    {
        ...
        if (!$item) { // if this kind of product has not been added to cart
    ...
    public function addStoregroup(Luciky_Sales_Model_Order_Storegroup $store_group){
    ...
    public function saveItemToStoregroup(Mage_Sales_Model_Quote_Item $item)
    {
    	...
    	//Mage::throwException('quoteid'.$_quote->getId().'hello');
    	...
    	//	Mage::throwException($this->getId());
			...
			if (!$store_group_item->getId()) {
				Mage::throwException('storegroup item has not been insert into table');
			}
    		
    	}else{
    		$store_group_item=$store_group->getFirstItem();
    	}
    	
    	$item->setData('sales_quote_storegroup_id',$store_group_item->getId());
    	try {
    		$item->save();
    	} catch (Exception $e) {
    		Mage::throwException('storegroupid has not insert into the quote item table');
    	}
