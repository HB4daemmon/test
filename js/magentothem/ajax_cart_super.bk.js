...
//         if($jq(this).find(".top-cart-content").css('position') == 'fixed') {
//             $jq(this).find(".top-cart-content").removeAttribute('style');
//             console.log('remove');
//         } else {
//             $jq(this).find(".top-cart-content").attr('style', 'position: fixed');
//             console.log('add');
//         }
//     });
//     $jq('#pinCart').click(function() {
//         if($jq(this).find(".top-cart-content").css('position') == 'fixed') {
//             $jq(this).find(".top-cart-content").removeAttribute('style');
//             console.log('remove');
//         } else {
...
function cutItemCount() {
    var is_checkout_page = $jq('.checkout-cart-index').length;
    if(is_checkout_page>0) return false;
    var linkToCart = $jq('.remove-item').attr('onclick');
    $jq('.remove-item').removeAttr('onclick')
    $jq('.remove-item').live('click', function(){
        linkToCart = linkToCart.replace("setLocation('","").replace("')","");
...
function addQty() {
    var linkToCart = $jq('#addQty').attr('onclick');
    $jq('#addQty').removeAttr('onclick')
    $jq('#addQty').live('click', function() {
        linkToCart = linkToCart.replace("setLocation('","").replace("')","");
...
function redQty() {
    var linkToCart = $jq('#redQty').attr('onclick');
    $jq('#redQty').removeAttr('onclick')
    $jq('#redQty').live('click',function() {
        linkToCart = linkToCart.replace("setLocation('","").replace("')","");
...
//    })
    ...
    function(){
       // alert('yes');
        $jq(".top-cart-content").attr('style', 'position: fixed;right:2px;top:300px;display:block;');
         $jq(".top-cart-content").attr('state', '1');
    },
    function(){
        $jq(".top-cart-content").attr('style', 'height:400px;display:block;');
