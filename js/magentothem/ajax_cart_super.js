                    ...
                    if(data.sidebar_cart) {
                        ...
                        if(mine=='view') {
                             if($jq('#ajax_cart_super_product_view').attr('class')=='popup') {
                                 ...
                                 window.parent.addQty();
                                 window.parent.redQty();
				 window.parent.Cartpin();
                            }
                        }
                    }
                    if(data.top_link){
                    ...
                    if(data.itemCount) {
                        $jq('#'+data.product_id).css('display', 'block');
                        var value = $jq('#'+data.product_id).attr('value');
                        $jq('#'+data.product_id).attr('value', '');
                        $jq('#'+data.product_id).attr('value', data.itemCount);
                        $jq('.inCart').each(function(){
                            if($jq(this).attr('id') == data.product_id) {
                                var value = $jq(this).attr('value');
                                $jq(this).attr('value', '');
                                $jq(this).attr('value', data.itemCount);
                                $jq(this).text('');
                                $jq(this).text(data.itemCount);
                                $jq(this).css('display', 'block');
                            }
                        })
                    } else {
                        $jq('#'+data.product_id).css('display', 'none');
                        $jq('#'+data.product_id).attr('value', 0);
                        $jq('.inCart').each(function(){
                            if($jq(this).attr('id') == data.product_id) {
                                var value = $jq(this).attr('value');
                                $jq(this).attr('value', '');
                    ...
                    //show minicart
                    if(data.mini_cart) {
                        var state=$jq('.top-cart-content').attr('state');
                        $jq('#mini_cart_block').html('');
                        $jq('#mini_cart_block').html(data.mini_cart);
                        if (state=='1') {
                            $jq(".top-cart-content").attr('style', 'position: fixed;right:30px;top:250px;display:block;');
                            $jq(".top-cart-content").attr('state', '1');
                            $jq(".mini-products-list").attr('style', 'max-height:500px; overflow: auto;');
                        };
...
function slideEffectAjax() {
      $jq('.top-cart-contain').mouseenter(function() {
        Cartpin();
          ...
          $jq(this).find(".top-cart-content").css('overflow', 'visible');
      });
      //hide submenus on exit
      $jq('.top-cart-contain').mouseleave(function() {
          if($jq(this).find(".top-cart-content").css('position') == 'fixed') {
          } else {
...
function Cartpin(){
    $jq('.pin').click(function(){
         ...
         if (state == '0') {
             ...
             $jq(".top-cart-content").attr('style', 'display:block;');
             $jq(".top-cart-content").attr('state', '0');
             $jq(".mini-products-list").attr('style', 'max-height:300px; overflow: auto;');
         }
...
function cutItemCount() {
    var is_checkout_page = $jq('.checkout-cart-index').length;
    if(is_checkout_page>0) return false;
    $jq('.remove-item').each(function(){
        var linkToCart = $jq(this).attr('onclick');
        $jq(this).removeAttr('onclick')
        $jq(this).live('click', function(){
            linkToCart = linkToCart.replace("setLocation('","").replace("')","");
            linkToCart =  linkToCart.replace('checkout/cart/add', 'ajaxcartsuper/ajaxcart/redCount');
            var e = null;
            // ajax code
            ajaxToCart(linkToCart,"",$jq(this));
            //End ajax code
...
function addQty() {
    $jq('#cart-sidebar a.addQty').each(function() {
        var linkToCart = $jq(this).attr('href');
        //$jq(this).removeAttr('onclick')
        $jq(this).attr('href','#');
        $jq(this).attr('onclick','');
        $jq(this).live('click', function() {
            linkToCart = linkToCart.replace("setLocation('","").replace("')","");
            linkToCart = linkToCart.replace('delete', 'addQty');
            $jq(this).attr('onclick',ajaxToCart(linkToCart,'','view'));
            //ajaxToCart(linkToCart,"",'view');
        });
    })
}

function redQty() {
    $jq('#cart-sidebar a.redQty').each(function() {
        var linkToCart = $jq(this).attr('href');
        //$jq(this).removeAttr('onclick')
        $jq(this).attr('href','#');
        $jq(this).attr('onclick','');
        $jq(this).live('click',function() {
            linkToCart = linkToCart.replace("setLocation('","").replace("')","");
            linkToCart = linkToCart.replace('delete', 'redQty');
            $jq(this).attr('onclick',ajaxToCart(linkToCart,'','view'));
            //ajaxToCart(linkToCart,"",'view');
        });
    })
...
function showRedButton() {
    $jq('.remove-item').each(function(){
        ...
        if($jq(this).attr('value') != 0 && $jq(this).attr('value')) {
            $jq(this).show();
        } else {
            $jq(this).hide();
        }
